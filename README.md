...
A cross-platform desktop app that runs AI coding CLIs in a secure, transparent, always-on-top terminal window **invisible to screen capture**.

Built with .NET 10 and Avalonia UI. Supports Windows and macOS.

## Features

- **Screen capture protection** — Window is excluded from screenshots and screen recording using OS-level APIs (`SetWindowDisplayAffinity` on Windows, `setSharingType` on macOS)
...
- **Built-in terminal** — Full xterm.js terminal powered by native PTY (ConPTY on Windows, forkpty on macOS)
- **Screenshot capture & injection** — Capture screen regions and inject them into the active CLI session
...
src/
  ...
  StealthPane.Terminal/     PTY library (ConPTY / forkpty)
```

- **MVVM** with CommunityToolkit.Mvvm source generators
- **Dependency injection** via Microsoft.Extensions.DependencyInjection
- **Messaging** with `WeakReferenceMessenger` for decoupled communication
- **Native interop** via `LibraryImport` for Win32 / Objective-C APIs
