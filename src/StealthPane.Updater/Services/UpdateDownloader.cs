...
{
    private const string AssetPattern = "StealthPane-";

    private readonly HttpClient http = new()
    {
        Timeout = TimeSpan.FromMinutes(10),
        DefaultRequestHeaders = { { "User-Agent", "StealthPane-Updater" } }
    };
        ...
        try
        {
            ...
            using (var archive = ZipFile.OpenRead(tempZip))
            {
                var launcherEntry = archive.Entries.FirstOrDefault(e =>
                    e.Name.Equals("stealthpane.exe", StringComparison.OrdinalIgnoreCase));
